using FluentValidation;
using Google.Protobuf.WellKnownTypes;
using Google.Rpc;
using Grpc.Core;

namespace PasswordBoxGrpcServer.Model.Exceptions
{
    internal static class RpcExceptionThrower
    {
        internal static void ThrowInvalidArgumentException(ValidationException ex)
        {
            var errors = ex.Errors;

            throw new Google.Rpc.Status
            {
                Code = (int)Code.InvalidArgument,
                Message = "Bad request",
                Details =
                    {
                        Any.Pack(new BadRequest
                        {
                            FieldViolations = {
                                errors.Select(item => new BadRequest.Types.FieldViolation{ Field = item.PropertyName, Description = item.ErrorMessage})
                            }
                        })
                    }
            }.ToRpcException();
        }

        internal static void ThrowInternalException(Exception ex)
        {
            throw new Google.Rpc.Status
            {
                Code = (int)Code.Internal,
                Message = "Debug Info",
                Details =
                {
                    Any.Pack(ex.ToRpcDebugInfo())
                }
            }.ToRpcException();
        }
    }
}
