using AuthService.Enums.EventId;
...
namespace AuthService.Services.Implementation
{
    public class AuthService : AuthProtoService.AuthProtoServiceBase
    {
        ...
        private const string DOMAIN = "AuthService";

        public AuthService(ILogger<AuthService> logger, IUserAuthenticator userAuthenticator, IUserRegistration userRegistration)
        {
            _logger = logger;
        ...
        public override async Task<AuthenticateUserReply> AuthenticateUser(UserRequest request, ServerCallContext context)
        {
            ...
            {
                var rpcEx = AuthRpcExceptions.InvalidArgumets(DOMAIN, ex.Errors);
